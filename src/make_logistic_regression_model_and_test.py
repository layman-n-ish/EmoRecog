...
def make_lg_final_model(X,Y):
	final_model = LogisticRegression(random_state=1, C = 3)
	final_model.fit(X,Y)
	return final_model
...
def store_model(final_model):
	joblib.dump(final_model, "Logistic_final.pkl")
def main():
	df = prepare_dataframe("linear_features.csv")
	data = prepare_data(df)
	X = data[0]
	Y = data[1]
	final_model = make_lg_final_model(X, Y)
